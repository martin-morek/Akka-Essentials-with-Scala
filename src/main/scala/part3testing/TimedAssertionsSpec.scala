...
class TimedAssertionsSpec extends
  TestKit(ActorSystem("TimeAssertionsSpec", ConfigFactory.load().getConfig("specialTimedAssertionsConfig")))
  with ImplicitSender
  with WordSpecLike
  ...
  "A worker actor" should {
    ...
    "reply with the meaning of line in a timely manner" in {
    ...
    "reply to a test in a timely manner" in {
      within(1 second) {
        val probe = TestProbe()
