...
class InterceptionLogsSpec extends
  TestKit(ActorSystem("InterceptionLogsSpec", ConfigFactory.load().getConfig("interceptingLogMessages")))
  with ImplicitSender
  with WordSpecLike
  ...
  val invalidCreditCart = "0000-0000-0000-0000"
  ...
  "A checkout flow" should {
    ...
    "freak out if the payment is denied" in {
      EventFilter[RuntimeException](occurrences = 1) intercept {
        val checkoutRef = system.actorOf(Props[CheckoutActor])
        checkoutRef ! Checkout(item, invalidCreditCart)
      }
    }
  }
